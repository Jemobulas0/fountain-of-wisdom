...
- **Item page goes live:** its icon must become an active link with the hero-style hover highlight everywhere it appears in rows (e.g. Counters). The only step is adding the item to `data/item-pages.json`. `item-links.js` then makes every `data-tooltip="item"` icon a link, and the highlight (`a.item-link:hover .build-tree-icon` in `data/tooltips.css`) applies automatically. Row icons must use `<span class="item-link" data-tooltip="item" data-item="{id}"><div class="build-tree-icon">...</div></span>`. Items without a page stay plain, no link and no highlight.
